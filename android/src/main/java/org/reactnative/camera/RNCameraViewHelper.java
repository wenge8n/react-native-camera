...
public class RNCameraViewHelper {
  ...
  public static void setExifData(ExifInterface exifInterface, ReadableMap exifMap) {
    ...
    if (exifMap.hasKey(ExifInterface.TAG_GPS_IMG_DIRECTION)) {
      Double direction = exifMap.getDouble(ExifInterface.TAG_GPS_IMG_DIRECTION);
      if (direction < 0) {
        direction += 360;
      }
      String GPSImgDirection_string = Math.round(direction * 100) + "/100";
      exifInterface.setAttribute(ExifInterface.TAG_GPS_IMG_DIRECTION, GPSImgDirection_string);
    }
  }
